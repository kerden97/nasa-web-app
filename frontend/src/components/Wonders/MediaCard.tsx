  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-0 px-4 pb-4 pt-20">
        <p className="font-nasa text-[11px] tracking-[0.18em] text-slate-400">{meta}</p>
        <p className="mt-2 text-xl font-semibold leading-tight text-white line-clamp-2">{title}</p>
        {teaser && <p className="mt-3 text-xs leading-6 text-slate-300 line-clamp-3">{teaser}</p>}
        <div className="mt-3 flex items-center justify-between gap-3 text-[11px]">
          <span className="truncate text-slate-400">{footerLeft}</span>
          <span className="whitespace-nowrap rounded-full border border-cyan-300/18 bg-white/5 px-3 py-1.5 font-medium text-white/90 transition-colors group-hover:border-cyan-300/30 group-hover:bg-cyan-400/10">
            View details
          </span>
        </div>
      </div>
